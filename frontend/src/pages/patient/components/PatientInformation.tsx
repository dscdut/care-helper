        ...
        <div className='flex flex-1 flex-col gap-2 lg:flex-row lg:items-center'>
          <ul className='flex h-full flex-col justify-between gap-2 lg:w-1/2'>
            <li className='flex gap-2'>
              ...
              <h3 className='font-bold lg:w-1/2'>{patientOfDoctor?.fullName}</h3>
            </li>
            <li className='flex gap-2'>
              <h3 className='lg:w-1/2'>Birthday</h3>
              <h3 className='font-bold lg:w-1/2'>{formatDate(patientOfDoctor?.birthday as string, 'DD/MM/YYYY')}</h3>
            </li>
            <li className='flex gap-2'>
              <h3 className='lg:w-1/2'>Gender</h3>
              <h3 className='font-bold lg:w-1/2'>{patientOfDoctor?.gender}</h3>
            </li>
            <li className='flex gap-2'>
              <h3 className='lg:w-1/2'>National Id Card</h3>
              <h3 className='font-bold lg:w-1/2'>{patientOfDoctor?.nationalIdCard}</h3>
            </li>
          </ul>
          <ul className='flex h-full flex-col gap-2 lg:w-1/2'>
            <li className='flex gap-2'>
              <h3 className='lg:w-1/3'>Address</h3>
              <h3 className='font-bold lg:w-2/3'>{patientOfDoctor?.address}</h3>
            </li>
            <li className='flex gap-2'>
              <h3 className='lg:w-1/3'>Job</h3>
              <h3 className='font-bold lg:w-2/3'>{patientOfDoctor?.profesion}</h3>
            </li>
          </ul>
        </div>
      </div>
      <div className='mt-4 flex w-full flex-col gap-5 lg:flex-row'>
        <div className='card flex-1 bg-white shadow-lg'>
          <div className='card-body gap-6'>
            ...
            <p className='whitespace-pre-line pl-2 leading-8'>{medicalHistory?.history}</p>
          </div>
        </div>
        <div className='flex flex-col gap-5'>
          ...
          <div className='card w-full min-w-max rounded-lg bg-slate-300 text-black shadow-lg lg:w-96'>
            <div className='card-body py-5'>
              ...
              <p>
                Height: <span className='font-bold'>{patientOfDoctor.height}</span>
              </p>
              <p>
                Weight: <span className='font-bold'>{patientOfDoctor.weight}</span>
              </p>
            </div>
          </div>
        </div>
